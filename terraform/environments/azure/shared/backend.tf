...
# Use terraform remote state for production envs. Here is an example how to use Azure storage account as backend.
...
/*terraform {
  backend "azurerm" {
    ...
    key                  = "terraform-shared-tfstate"                        # Unique key per environment
  }
}
*/